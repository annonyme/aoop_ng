...
if(isset($request["groupId"]) && $_SESSION["XWUSER"]->isInGroup("admins") && XWAddonManager::instance()->getAddonByName("XWUserSession")->checkSessionSecToken()){
   ...
   if($group->getName()!="admins" || $group->getId()==0){
       $group->setName($request["groupName"]);	
   }
   
   $group->setDescription($request["groupDescription"]);
   
   $group->save();
   ?>
   <div class="PresentationBoxHeader">Saved</div>
   <div class="PresentationBox">Group was saved.</div>
   <?php
} 
else{
	?>
