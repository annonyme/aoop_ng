...
class AddToGroup extends Command{
    protected function configure(){
        $this->setName('aoop:users:groups:add');
        $this->setDescription('Add user to usergroup');
        ...
        if($user->getId() > 0){
            ...
            for($i=0;$i<$groups->getSize();$i++){
                if($groups->getGroup($i)->getName() == $input->getOption('group')){
                    $groups->getGroup($i)->addUser($user);
                    $output->writeln("user was added to group");
                }
            }
